...
Now that you have your starting terrain, a settlement, and a dungeon, it is time to place major features in the surrounding hexes. The below simulates a borderlands of civilization where adventure happens.
...
First, determine the threat rank of the area. You can presume that any hex adjacent to a “settled” hex (one that is secured or controlled) is equivalent to “level 1” of a dungeon. For each hex beyond that, increase the level. For areas that cover multiple hexes, you can take the lowest or the average value since the threat rank tables have overlap built in.
...
Finally, once you know the general category of creature and the (Equivalent) Hit Die range, select or otherwise determine the one you wish to use (see Appendix A).  Enter the resulting creature to your encounter table, taking care to note the “number appearing” and “percent in lair” values.

You will repeat the process until you have a filled out encounter chart (typically 6 entries to start). You may wish to pick creatures along a theme, allowing that first determination to set the tone for the region. You may also want to reserve one entry for pitfalls, quicksand, or other tricks/phenomena. 

#### Number Appearing and % in Lair

When stocking a wilderness, use the number appearing and guidelines in volume II or Appendix A. The numbers of creatures in the wilds are not constrained by the limits of dungeon corridors. 
...
Want your Stronghold ruled by an Elf Lord, Orc Bandit King, Vampire Queen, etc?. Imagination is the limit.
...
A Stronghold is a special kind of settlement, typically established by high level NPCs. It can be a Warrior’s Keep, a Sorcerer’s Tower, an Outlaw’s Hideout, etc. Like Towns, they secure the Hex in which they are located. A stronghold is defined by its occupant(s), which are generated using the rules for NPCs, with a level of 9+1d6 (or use the quick list below)
...
- Strongholds will be protected by 3d6x10 1HD warriors, half outfitted with Crossbows and Light Armor, and the rest with Heavy Armor and Polearms (or similar forces). For every 30 soldiers, there is also a sergeant of 4th level.
